...
Sim.n_cest_pool   = 4;                    % number of CEST/NOE pools (CEST pools: B,D,E,F,G)
...
Sim.n_cest_pool=2;
Sim.fD=0.0017;  % rel. conc 190mM/111M
...
Sim.R2D=1/8e-4;     % R2B relaxation rate [Hz]

% third CEST pool E (PCr)
Sim.n_cest_pool=3;
Sim.fE=0.0016;  % rel. conc 180mM/111M
Sim.kEA=160;   % exchange rate in Hz ( the fast one, kBA is calculated by this and fB)
Sim.dwE=2.6;     % ppm  relative to dwA
Sim.R1E=1;      % R1B relaxation rate [Hz]
Sim.R2E=1/1.5e-3;     % R2B relaxation rate [Hz]

% forth CEST pool F (NOE, fat)
Sim.n_cest_pool=4;
Sim.fF=0.0003;  % rel. conc 35mM/111M
Sim.kFA=25;   % exchange rate in Hz ( the fast one, kBA is calculated by this and fB)
Sim.dwF=-3.5;     % ppm  relative to dwA
Sim.R1F=1;      % R1B relaxation rate [Hz]
Sim.R2F=1/1e-3;     % R2B relaxation rate [Hz]
...
T.lowerA      = [   Sim.dwA-0.05     1/4*Sim.R1A   Sim.R2A*0.1    ];
T.upperA      = [   Sim.dwA+0.05     2*Sim.R1A     Sim.R2A*10  ];
...
T.lowerB      = [   Sim.dwB-0.05    Sim.fB*0.01  Sim.kBA/10     Sim.R2B/10      ];
T.upperB      = [   Sim.dwB+0.05    Sim.fB*100    Sim.kBA*10    Sim.R2B*100     ];
...
T.upperC      = [0        0.1         10000       1/0.1       100*Sim.R2C       ];
...
T.lowerD      = [Sim.dwD-0.05   Sim.fD*0.01    Sim.kDA/10     Sim.R2D/10            ];
T.upperD      = [Sim.dwD+0.05   Sim.fD*100     Sim.kDA*10    Sim.R2D*100            ];
...
T.lowerE      = [Sim.dwE-0.05   Sim.fE*0.01    Sim.kEA/10     Sim.R2E/10            ];
T.upperE      = [Sim.dwE+0.05   Sim.fE*100     Sim.kEA*10    Sim.R2E*100            ];
...
T.lowerF      = [Sim.dwF-0.05   Sim.fF*0.01    Sim.kFA/10     Sim.R2F/10            ];
T.upperF      = [Sim.dwF+0.05   Sim.fF*100     Sim.kFA*10    Sim.R2F*100            ];
