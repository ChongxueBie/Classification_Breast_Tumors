...
def mmontage( d, cmap=gray ):
	...
	ii = 0
	d2 = zeros( (int(im_rows*rows), int(im_cols*cols)), dtype=d.dtype)
...
def cestFit( freq, mm, fitinds, newfreq=None ):
	...
	def lorentzian_min(x, freq, data):
		...
		# return (lorentzian(freq, x[0], x[1], x[2], x[3], x[4]) - (data))
	...
	def lorentzianFit(freq, data):
		...
		# x, success = leastsq(lorentzian_min, x0=(A,x0,w, b, k), args=(freq, data))
