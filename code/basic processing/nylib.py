...
def searchFileNext(file, string):
    with open(file) as f:
        lines = f.readlines()
    for index, line in enumerate(lines): # get the index for each line as we go
        if string in line: # if this isn't the first line & is just '100'
            return (lines[index+1]).strip() # print the previous line
